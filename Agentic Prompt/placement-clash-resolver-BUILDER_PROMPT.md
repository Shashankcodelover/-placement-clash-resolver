...
Domain of this project: scheduling conflict detection and resolution for placement/interview slots
...
of how comparable tools in scheduling conflict detection and resolution for placement/interview slots solve the same problem, what they
...
non-negotiable; any animation degrades gracefully. The user is under real time pressure trying to resolve a scheduling conflict. The conflict itself and the resolution options must be obvious at a glance -- no digging through menus to understand what clashed and why.
...
network, concurrent use, interrupted sessions. Trace the full conflict-detection-to-resolution flow, including edge cases like overlapping slots by seconds, back-to-back slots, and simultaneous conflicting requests from two users.
...
Dependencies free of known critical vulnerabilities. If this touches other students' schedules or personal data, make sure one user can never see or modify another user's slot data without authorization.
...
GitHub repo (not in use during local-only phase): https://github.com/Shashankcodelover/-placement-clash-resolver.git
